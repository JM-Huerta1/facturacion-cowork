...
const TIPO_COLORS = { A: '#E6F1FB', B: '#FFF3CD', C: '#f5f5f5', X: '#EEEDFE', I: '#FAECE7', ND: '#FCEBEB' }
...
export default function Mensual() {
  ...
  const [solapa, setSolapa] = useState('pendiente') // 'pendiente' | 'facturado'
  ...
  async function proyectar() {
    ...
    for (const comp of recurrentes) {
      const itemsOrigen = (comp.comprobante_items || [])
        .map(it => `${it.descripcion}__${it.cantidad}`)
        .sort().join('|')

      const yaExiste = (existentesProx || []).some(ex =>
        ex.cliente_id === comp.cliente_id &&
        ex.sede_id === comp.sede_id &&
        ex.tipo === comp.tipo &&
        (ex.comprobante_items || [])
          .map(it => `${it.descripcion}__${it.cantidad}`)
          .sort().join('|') === itemsOrigen
      )
      ...
      const { data: nuevo, error } = await supabase
        .from('comprobantes')
        .insert({
          sede_id: comp.sede_id,
          cliente_id: comp.cliente_id,
          tipo: comp.tipo,
          fecha: fechaProx,
          recurrente: true,
          estado: 'pendiente',
          notas: tieneProporcional
            ? `Proyectado desde ${comp.fecha} · ⚠ Tenía proporcional — verificar importe`
            : `Proyectado desde ${comp.fecha}`
        })
        .select('id')
        .single()
      ...
      const itemsActualizados = (comp.comprobante_items || []).map(it => {
        ...
        return {
          ...
          precio_neto: prodActual ? prodActual.precio_neto : it.precio_neto,
          descuento_pct: it.descuento_pct,
          proporcional_pct: 100,
      ...
      if (itemsActualizados.length > 0) {
        await supabase.from('comprobante_items').insert(itemsActualizados)
      }
      creados++
    }
  ...
  async function exportarCSV() {
    const paraExportar = exportarTodo
      ? comprobantes
      : comprobantes.filter(c => c.estado === 'pendiente')

    if (!paraExportar.length) {
      return setMensaje(exportarTodo ? 'No hay comprobantes en este mes' : 'No hay comprobantes pendientes para exportar')
    }
    ...
    for (const comp of paraExportar) {
      ...
      for (const it of comp.comprobante_items || []) {
        ...
        rows.push([
          ...
          c?.razon_social || '',
          it.descripcion,
          Math.round(Number(it.precio_neto)),
          it.cantidad,
          Math.round(base),
          it.alicuota_iva,
          Math.round(iva_monto),
          Math.round(base + iva_monto),
          it.cuenta_contable || '',
          s?.nombre || ''
        ])
      }
    ...
    a.href = url
    const sufijo = exportarTodo ? 'todas' : 'pendientes'
    a.download = `colppy_${MESES[mes].toLowerCase()}_${anio}_${sufijo}.csv`
    a.click()
  ...
  if (!usuario) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', color: '#888', fontFamily: 'sans-serif' }}>
      Cargando...
    </div>
  )
    ...
    <Layout usuario={usuario}>
      <div style={{ padding: '32px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            {/* Toggle exportar todo / solo pendientes */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 12px', background: '#f5f5f5', borderRadius: '8px', border: '1px solid #e0e0e0' }}>
              ...
              <button
                onClick={() => setExportarTodo(false)}
                style={{ padding: '4px 10px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: '600', background: !exportarTodo ? '#FFF3CD' : 'transparent', color: !exportarTodo ? '#854F0B' : '#888' }}
              >
                Pendientes
              </button>
              <button
                onClick={() => setExportarTodo(true)}
                style={{ padding: '4px 10px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: '600', background: exportarTodo ? '#E6F1FB' : 'transparent', color: exportarTodo ? '#185FA5' : '#888' }}
              >
                Todas
              </button>
            </div>
            <button onClick={exportarCSV} style={{ padding: '9px 16px', border: '1px solid #e0e0e0', borderRadius: '8px', background: 'white', cursor: 'pointer', fontSize: '13px', color: '#555', fontWeight: '500' }}>
              ↓ Exportar CSV Colppy
            </button>
        ...
        <div style={{ display: 'flex', gap: '0', marginBottom: '0', borderBottom: '2px solid #e8e8e8' }}>
          {[
            { key: 'pendiente', label: 'Pendientes de facturar', count: pendientes.length, color: '#854F0B', bg: '#FFF3CD', active_border: '#F59E0B' },
            { key: 'facturado', label: 'Facturados', count: facturados.length, color: '#2E7D32', bg: '#E8F5E9', active_border: '#27ae60' },
          ].map(tab => (
            <button
              key={tab.key}
              ...
              style={{
                padding: '12px 24px',
                border: 'none',
                borderBottom: solapa === tab.key ? `3px solid ${tab.active_border}` : '3px solid transparent',
                background: 'white',
                cursor: 'pointer',
                fontSize: '13px',
            ...
            >
              ...
              <span style={{
                ...
                fontSize: '11px',
                fontWeight: '700',
                background: solapa === tab.key ? tab.bg : '#f5f5f5',
                color: solapa === tab.key ? tab.color : '#888',
              }}>
                {tab.count}
              </span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', margin: '16px 0', flexWrap: 'wrap' }}>
          ...
          <input
            ...
            style={{ padding: '7px 12px', border: '1px solid #e0e0e0', borderRadius: '8px', fontSize: '13px', width: '200px', color: '#333' }}
          />
          <select
            value={sedeFiltro}
            onChange={e => setSedeFiltro(e.target.value)}
            style={{ padding: '7px 12px', border: '1px solid #e0e0e0', borderRadius: '8px', fontSize: '13px', background: 'white', color: '#555' }}
          >
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            <thead>
              <tr style={{ background: '#FAFAFA', borderBottom: '1px solid #f0f0f0' }}>
                {['Fecha', 'Cliente', 'Sede', 'Tipo', 'Ítems', 'Total', 'Recurrente', ''].map(h => (
            ...
            <tbody>
              {filtrados.length === 0 && (
                <tr>
                  <td colSpan={8} style={{ padding: '48px', textAlign: 'center', color: '#bbb', fontSize: '14px' }}>
                    {solapa === 'pendiente'
                      ? '✓ No hay comprobantes pendientes de facturar'
                      : 'No hay comprobantes facturados en este período'
                    }
                  </td>
                </tr>
              )}
              {filtrados.map(comp => {
                ...
                return (
                  <tr key={comp.id} style={{ borderBottom: '1px solid #f5f5f5' }}>
                    <td style={{ padding: '12px 16px', color: '#666' }}>{comp.fecha}</td>
                    ...
                    <td style={{ padding: '12px 16px', color: '#666', fontSize: '12px' }}>{sede?.nombre || '—'}</td>
                    <td style={{ padding: '12px 16px' }}>
                    ...
                    <td style={{ padding: '12px 16px', fontSize: '12px', maxWidth: '200px' }}>
                      ...
                      {tieneProp && (
                        <span style={{ background: '#FFF3CD', color: '#854F0B', padding: '1px 6px', borderRadius: '4px', fontSize: '10px', fontWeight: '600', whiteSpace: 'nowrap' }}>
                          ⚠ Prop.
                        </span>
                      )}
                    </td>
                    <td style={{ padding: '12px 16px', fontWeight: '700', color: '#1a1a2e' }}>{formatMoney(total)}</td>
                    <td style={{ padding: '12px 16px', textAlign: 'center' }}>
                      <button
                        onClick={() => toggleRecurrente(comp.id, comp.recurrente)}
                        style={{ width: '40px', height: '22px', borderRadius: '11px', border: 'none', background: comp.recurrente ? '#27ae60' : '#ddd', cursor: 'pointer', position: 'relative', transition: 'background .2s' }}
                        title={comp.recurrente ? 'Quitar recurrencia' : 'Marcar como recurrente'}
                      >
                        <span style={{ position: 'absolute', top: '3px', left: comp.recurrente ? '20px' : '3px', width: '16px', height: '16px', borderRadius: '50%', background: 'white', transition: 'left .2s', display: 'block' }} />
                      </button>
                    </td>
                    <td style={{ padding: '12px 16px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                      ...
                      {solapa === 'pendiente'
                        ? <button onClick={() => marcarFacturado(comp.id)} style={{ padding: '5px 12px', border: '1px solid #C8E6C9', borderRadius: '6px', background: '#F1F8F1', cursor: 'pointer', fontSize: '12px', color: '#2E7D32', fontWeight: '600' }}>✓ Facturado</button>
      ...
      {verComp && (
        <div
          ...
          onClick={() => setVerComp(null)}
        >
          <div
            style={{ background: 'white', borderRadius: '12px', padding: '28px', width: '660px', maxWidth: '95vw', maxHeight: '85vh', overflowY: 'auto' }}
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div>
                ...
                <div style={{ fontSize: '12px', color: '#888', marginTop: '3px' }}>
                  {clientes[verComp.cliente_id]?.cuit} · {sedes[verComp.sede_id]?.nombre}
                </div>
              </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', marginBottom: '16px' }}>
              <thead>
                <tr style={{ background: '#FAFAFA', borderBottom: '1px solid #f0f0f0' }}>
                  {['Descripción', 'Cant.', 'Precio neto', 'Dto %', 'Prop %', 'IVA', 'Subtotal'].map(h => (
              ...
              <tbody>
                {(verComp.comprobante_items || []).length === 0 && (
                  <tr><td colSpan={7} style={{ padding: '20px', textAlign: 'center', color: '#bbb' }}>Sin ítems</td></tr>
                )}
                ...
                return (
                  <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', color: '#666', marginBottom: '4px' }}>
                      <span>Subtotal neto</span><span>{formatMoney(neto)}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', color: '#666', marginBottom: '8px' }}>
                      <span>IVA</span><span>{formatMoney(iva)}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '16px', fontWeight: '700', color: '#1a1a2e', borderTop: '1px solid #dde6f5', paddingTop: '8px' }}>
                      <span>Total</span><span style={{ color: '#378ADD' }}>{formatMoney(neto + iva)}</span>
                    </div>
                  </>
                )
              })()}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                {(verComp.estado || 'pendiente') === 'pendiente'
                  ? <button onClick={() => { marcarFacturado(verComp.id); setVerComp(null) }} style={{ padding: '8px 16px', border: '1px solid #C8E6C9', borderRadius: '8px', background: '#F1F8F1', cursor: 'pointer', fontSize: '13px', color: '#2E7D32', fontWeight: '600' }}>✓ Marcar como facturado</button>
                ...
                <button onClick={() => setVerComp(null)} style={{ padding: '8px 16px', border: '1px solid #e0e0e0', borderRadius: '8px', background: 'white', cursor: 'pointer', fontSize: '13px', color: '#555' }}>
                  Cerrar
                </button>
              </div>
            </div>
